...
template <>
void BasicAttributeFilter<StringAttributeCallback>(TLV::TLVReader * data, Callback::Cancelable * onSuccess,
                                                   Callback::Cancelable * onFailure)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    const uint8_t * val;
    uint32_t len;

    if (data->GetType() != TLV::kTLVType_ByteString && data->GetType() != TLV::kTLVType_UTF8String)
    {
    ...
    if (CHIP_NO_ERROR == err)
    {
        LogStringAttribute(val, len, data->GetType() == TLV::kTLVType_UTF8String);

        Callback::Callback<StringAttributeCallback> * cb = Callback::Callback<StringAttributeCallback>::FromCancelable(onSuccess);
        cb->mCall(cb->mContext, ByteSpan(val, len));
    }
    else
    {
        Callback::Callback<DefaultFailureCallback> * cb = Callback::Callback<DefaultFailureCallback>::FromCancelable(onFailure);
